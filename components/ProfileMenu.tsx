    ...
    <div className="relative">
      <button
        ...
        className={`inline-flex size-10 items-center justify-center rounded-full ${focus} ${
          open || active ? "bg-[#ecebfb] text-[#3b3499]" : "text-neutral-600 hover:bg-[#f0f0ee] hover:text-neutral-900"
        }`}
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full z-50 mt-2 w-72 rounded-2xl border border-[#e4e3de] bg-white p-4 shadow-lg">
            {isPending ? (
              <p className="text-sm text-neutral-500">Loading…</p>
            ) : anonymous ? (
              showAuthOptions ? (
                <>
                  <p className="text-[15px] font-semibold text-[#222]">Create an account</p>
                  <p className="mt-1 text-sm text-neutral-500">Choose how to sign in:</p>
                  <div className="mt-3 flex flex-col items-stretch gap-2">
                    {authOptions?.google && (
                      <button
                        ...
                        className={`inline-flex min-h-10 items-center justify-center gap-2 rounded-full border border-[#d5d5d1] bg-white px-4 text-sm font-medium text-[#333] hover:bg-[#f7f7f5] ${focus}`}
                      >
                ...
                <>
                  <p className="text-[15px] font-semibold text-[#222]">No account yet</p>
                  ...
                  <button
                    type="button"
                    onClick={showOptions}
                    className={`mt-3 min-h-10 w-full rounded-full bg-[var(--accent)] px-4 text-sm font-semibold text-white ${focus}`}
                  >
                    Create an account
                  </button>
                </>
              ...
              <>
                <p className="truncate text-[15px] font-semibold text-[#222]">{user.name || "Account"}</p>
                ...
                <button
                  ...
                  className={`mt-3 text-sm text-neutral-600 underline underline-offset-2 hover:text-neutral-900 ${focus}`}
                >
                  Sign out
                </button>
              </>
            )}
            <div className="mt-4 border-t border-[#ecebe7] pt-3">
              <Link
                href="/settings"
                onClick={close}
                className={`inline-flex min-h-9 items-center text-sm font-medium text-[var(--accent)] hover:underline ${focus}`}
              >
                Settings
              </Link>
            </div>
