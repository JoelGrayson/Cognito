  ...
  return (
    <div className="wb flex min-h-screen flex-col">
      <header className="mx-auto flex w-full max-w-6xl items-center px-5 py-5 sm:px-8">
        <span className="text-base font-medium tracking-tight">Cognito</span>
      </header>
      ...
      <main className="flex-1">
        <section className="mx-auto grid w-full max-w-6xl items-center gap-10 px-5 pb-14 pt-6 sm:px-8 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:gap-16 lg:pb-24 lg:pt-14">
          <div>
            ...
            <div className="mt-8 flex flex-col items-start gap-3">
              <Link
                href={cta.href}
                className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-(--wb-primary) px-8 text-lg font-medium text-(--wb-card) transition-colors hover:opacity-90 focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-accent-strong sm:w-auto"
              >
                {cta.label}
          ...
          <div className="rounded-2xl border border-(--wb-line) bg-(--panel) p-4 sm:p-6">
        ...
        <section aria-labelledby="problems" className="border-t border-(--wb-line) bg-(--panel)">
          <div className="mx-auto w-full max-w-6xl px-5 py-14 sm:px-8 lg:py-20">
            ...
            <ul className="mt-8 grid gap-4 md:grid-cols-3">
              {PROBLEMS.map((item) => (
                <li key={item.q} className="rounded-xl border border-(--wb-line) bg-(--wb-card) p-5">
          ...
          <ol className="mt-8 grid gap-6 md:grid-cols-3">
            {STEPS.map((step, i) => (
              <li key={step.title} className="flex gap-4">
                <span
                  aria-hidden="true"
                  className="flex size-9 flex-none items-center justify-center rounded-full bg-(--wb-butter) font-medium text-(--wb-butter-ink)"
                >
                  {i + 1}
                </span>
                <div>
